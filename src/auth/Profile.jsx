...
const Profile = () => {
  ...
  const save = async () => {
    ...
    if (!emailRegex.test(profile.email)) {
      show("Please enter a valid email format (e.g., user@gmail.com)", "error");
      return;
    }
  ...
  const submitDeletion = async () => {
    ...
    setDeleting(true);
    try {
      await api.post("/users/me/request-deletion", deleteForm);
      const res = await api.get("/users/me"); // ← add this
      setProfile(res.data); // ← add this
      show(
  ...
  const daysLeftToDelete = () => {
    const requested = parseDate(profile?.deletionRequestedAt);
    if (!requested) return null;
    const deadline = new Date(requested.getTime() + 7 * 24 * 60 * 60 * 1000);
    const diff = deadline - new Date();
    return Math.max(0, Math.ceil(diff / (1000 * 60 * 60 * 24)));
  };

  const deleteModalJsx = showDeleteModal && (
    <div
    ...
    >
      ...
      >
        ...
        <div style={{padding: "8px 4px"}}>
          ...
          >
            This action is permanent. Your account will be deleted after a 7-day
            grace period.
          </p>
          ...
          <input
            ...
            onChange={(e) =>
              setDeleteForm({...deleteForm, fullName: e.target.value})
            }
          ...
          <input
            ...
            onChange={(e) =>
              setDeleteForm({...deleteForm, phone: e.target.value})
            }
          />
  ...
  if (!profile) return <UserLayout>No profile found</UserLayout>;

  /* ================= COMPLETED PROFILE VIEW ================= */
  if (profile.profileCompleted && !editMode) {
    return (
      <UserLayout>
          ...
          <div className="profile-card">
            ...
            <div className="pv-section">
              <div className="pv-section-head">
                <span className="pv-section-title">Personal Info</span>
                <button
              ...
              <div className="pv-fields">
                <div className="pv-field">
                  ...
                  <span className="pv-field-val">{profile.name}</span>
                </div>
                <div className="pv-field">
                  <span className="pv-field-label">Email</span>
                  <span className="pv-field-val">{profile.email}</span>
                </div>
                <div className="pv-field">
                  <span className="pv-field-label">Phone</span>
                  <span className="pv-field-val">{profile.phone}</span>
                </div>
                <div className="pv-field">
                  <span className="pv-field-label">City</span>
                  <span className="pv-field-val">{profile.city}</span>
                </div>
              </div>
            </div>

            <div className="pv-section" style={{marginTop: "20px"}}>
              <div className="pv-section-head">
                <span
              ...
              <div className="pv-fields">
                <div className="pv-field">
                  <span className="pv-field-label">Language</span>
                  <select
                    value={selectedLang}
            ...
            <div className="pv-section">
              <div className="pv-section-head">
                <span className="pv-section-title" style={{color: "#dc2626"}}>
                  Danger Zone
                </span>
              </div>
              <p
              ...
              >
                Deleting your account is permanent. You'll have 7 days to change
                your mind before it's finalized.
              </p>
              <button
                className="pv-edit-btn"
      ...
      <div className="profile-container">
        <div className="profile-card">
          ...
          <div className="profile-name">
            {profile.profileCompleted ? profile.name : "Complete Your Profile"}
          </div>
          ...
          <div className="profile-completion warning">
            <div className="profile-completion-row">
              ...
              <span
                className={
                  completion === 100 ? "completion-pct done" : "completion-pct"
                }
              >
          ...
          <div className="profile-section">
            ...
            <label>Phone</label>
            <input className="profile-input" value={profile.phone} disabled />
            ...
            {photoInfo && (
              <div className="file-meta">
                {photoInfo.name} ({photoInfo.size} KB)
              </div>
            )}
            ...
            {profile.profilePicUrl && (
              <>
                ...
                <span
                  ...
                  onClick={() => setPreviewImage(profile.profilePicUrl)}
                >
                  View Photo
                </span>
              </>
            ...
            {idProofInfo && (
              <div className="file-meta">
                {idProofInfo.name} ({idProofInfo.size} KB)
              </div>
            )}
            ...
            {profile.idProofUrl && (
              <>
                ...
                <span
                  className="current-file-link"
                  onClick={() => setPreviewImage(profile.idProofUrl)}
                >
                  View ID Proof
                </span>
              </>
          ...
          <div className="profile-actions">
            <button
              ...
              onClick={save}
              disabled={completion < 100 || saving}
            >
              {saving ? (
                <>
                  <span className="profile-save-spinner"></span>Saving…
                </>
              ) : (
                "Save Changes"
              )}
